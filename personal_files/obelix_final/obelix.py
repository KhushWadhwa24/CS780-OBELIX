...
class FinderEnv:
    ...
    def reset(self):
        ...
        self.env.enable_push = False

        return obs

    def step(self, action):
        obs, reward, done = self.env.step(action, render=True)
...
class PusherEnv:
    ...
    def reset(self, seed: Optional[int] = None):
        ...
        bx, by = self.env.bot_center_x, self.env.bot_center_y

        theta = np.deg2rad(self.env.rng.integers(0, 360))

        overlap = self.env.bot_radius + self.env.box_size // 2 - 9

        self.env.box_center_x = int(bx + overlap * np.cos(theta))
        self.env.box_center_y = int(by + overlap * np.sin(theta))

        self.env._update_frames(show=False)
        self.env.get_feedback()
        self.env.check_done_state()

        return self.env.sensor_feedback.copy()
...
class UnwedgeEnv:
    ...
    def reset(self):
        ...
        if not self.env.wall_obstacles:
            raise ValueError("UnwedgeEnv requires wall_obstacles=True")

        obstacles = self.env.obstacles

        (x1, _), (x2, _) = obstacles[0]

        top_wall = min(obstacles, key=lambda r: r[0][1])
        bottom_wall = max(obstacles, key=lambda r: r[0][1])

        gap_top = top_wall[1][1]
        gap_bottom = bottom_wall[0][1]

        bot_r = self.env.bot_radius
        h = self.env.frame_size[0]

        margin = 0  # prevent spawning inside wall

        if self.env.rng.integers(0, 2) == 0:
            self.env.bot_center_x = x1 - bot_r - margin
            self.spawn_side = "left"
            angle_offset = self.env.rng.uniform(-90, 90)
            self.env.facing_angle = angle_offset

        else:
            self.env.bot_center_x = x2 + bot_r + margin
            self.spawn_side = "right"
            angle_offset = self.env.rng.uniform(-90, 90)
            self.env.facing_angle = 180 + angle_offset

        choose_upper = self.env.rng.integers(0, 2)

        if choose_upper:
            y_low = 10 + bot_r + 2
            y_high = gap_top - bot_r - 2
        else:
            y_low = gap_bottom + bot_r + 2
            y_high = h - 10 - bot_r - 2
        ...
        self.env.bot_center_y = int(self.env.rng.integers(y_low, y_high))

        self.env.stuck_flag = 0
    ...
    def step(self, action):
        ...
        mid_x = self.env.frame_size[1] // 2

        if self.spawn_side == "left":
            if self.env.bot_center_x > mid_x + 20:
                done = True
        ...
        reward += 2000 if done else 0

        return obs, reward, done

    def update_state_diagram(self):
        self.env.update_state_diagram()
