...
def main():
    ...
    eta = args.eta  # intrinsic reward weight
    steps = 0
    ...
    for ep in range(start_ep, args.episodes):
        ...
        for _ in range(args.max_steps):
            ...
            if steps % 4 == 0 and len(replay) > max(args.warmup, args.batch * args.seq_len):
                ...
                # --- Forward pass ---
                q_vals, _ = q(states)                 # (B, T, A)
                next_q_online, _ = q(next_states)     # (B, T, A)
                next_q_target, _ = tgt(next_states)   # (B, T, A)

                # --- Double DQN ---
                next_actions = torch.argmax(next_q_online, dim=2)  # (B, T)
                ...
                # --- Targets ---
                y = rewards + args.gamma * (1 - dones) * next_val  # (B, T)
